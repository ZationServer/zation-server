...
class SmallBag
{
    ...
    {
        let groups = this._zc.getApp(Const.App.KEYS.USER_GROUPS)[Const.App.USER_GROUPS.AUTH];
        for(let k in groups)
        {
            if(groups.hasOwnProperty(k))
            {
                this.publishToAuthUserGroup(groups[k],eventName,data,cb);
            }
        }
    }
    ...
    // noinspection JSUnusedGlobalSymbols
    publishToCustomChannel(channel, id, eventName, data, cb)
    {
        this._exchangeEngine.publishToCustomChannel(channel,id,eventName,data,cb);
    }

    // noinspection JSUnusedGlobalSymbols
    pubCustomChannel(channel, id, eventName, data, cb)
    {
        this.publishToCustomChannel(channel,id,eventName,data,cb);
    }
