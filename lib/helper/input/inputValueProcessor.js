...
class InputValueProcessor
{
    ...
    static _processValue(input,config,currentInputPath,errorBag)
    {
        ...
        {
            //Object
            return InputValueProcessor._processObject(input,config,currentInputPath,errorBag);
        }
        else
        {
            //normal Input
    ...
    static _processArray(input,config,currentInputPath,errorBag)
    {
        ...
        {
            let isObjectArray = config.hasOwnProperty(Const.App.OBJECTS.PROPERTIES);
            ...
            for(let i = 0; i < input.length; i++)
            {
                ...
                {
                    //array normal item
                    input[i] = InputValueProcessor._processData(input[i],config,currentInputPathNew,errorBag);
                }
            }
            return input;
        }
