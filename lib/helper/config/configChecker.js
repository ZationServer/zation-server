...
class ConfigChecker
{
   ...
   _checkServices()
   {
       let s = this._zc.getMain(Const.Main.KEYS.SERVICES);

       //check services
       if(typeof s === 'object')
       {
           ConfigCheckerTools.assertStructure
           (Structures.Services,s,`Services: `,Const.Settings.CN.MAIN,this._ceb);
   ...
   {
       let cs = this._zc.getMain(Const.Main.KEYS.CUSTOM_SERVICES);
       //check custom services
       if(typeof cs === 'object')
       {
           for(let serviceName in cs)
           {
               if(cs.hasOwnProperty(serviceName))
               {
                   ...
                   if(typeof cs[serviceName] === 'object')
                   {
                       ...
                       ConfigCheckerTools.assertProperty
                       (
                           Const.Main.CUSTOM_SERVICES.CREATE,
                           service,
                           'function',
                           false,
                       ...
                       ConfigCheckerTools.assertProperty
                       (
                           Const.Main.CUSTOM_SERVICES.GET,
                           service,
                           'function',
                           true,
                           `Custom Services: '${serviceName}' `,
                       ...
                       for(let k in service)
                       {
                           if(service.hasOwnProperty(k))
                           {
                               if(k === Const.Main.CUSTOM_SERVICES.GET ||
                                   k === Const.Main.CUSTOM_SERVICES.CREATE)
                               {
                                   continue;
                               }
                               ConfigCheckerTools.assertProperty
