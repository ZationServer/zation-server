...
main config
second layer for ex. https , services etc
 */
...
class ConfigChecker
{
   ...
   _checkInputBody(value,target,objName)
   {
       ...
       {
           if(!this._objectsConfig.hasOwnProperty(value))
           {
               this._ceb.addConfigError(new ConfigError(Const.Settings.CN.APP,
                   `${target} the dependency to object: '${value}' can not be resolved, Object not found.`));
           }
           else
           {
               ...
               else if(objName !== undefined)
               {
                   ...
                   this._objectImports.push({o:objName,i:value});
               }
           }
       }
