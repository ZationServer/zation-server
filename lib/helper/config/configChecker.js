                ...
                input[keys[i]][Const.App.INPUT.IS_OPTIONAL])
            {
                if((keys.length-1) !== i && !wasLastOptional)
                {
                    ...
                    (
                        ...
                        `${target} input: '${keys[i]}', It is recommended to set the optional parameters at the first input level at the end.`
                    );
                    break;
                }
   ...
   _checkInputBody(value,target,objName)
   {
       ...
       else if(Array.isArray(value))
       {
           ...
           else
           {
               ...
               {
                   this._checkPropertyByObjLink(name,target,objName);
               }
               else
               {
                   this._ceb.addConfigError(new ConfigError(Const.Settings.CN.APP,
       ...
       else if(typeof value === "object")
       {
           ...
           if(value.hasOwnProperty(Const.App.OBJECTS.PROPERTIES))
           {
               ...
               {
                   //with is array
                   structure = Structures.ObjBody;
               }
               else
               {
                   //without is array
                   structure = Structures.AppObject;
               }

               //isObject
               ConfigCheckerTools.assertStructure(structure,value,target,Const.Settings.CN.APP,this._ceb);
               if(typeof value[Const.App.OBJECTS.PROPERTIES] === 'object')
               {
                   ...
                   for(let k in props)
                   {
                       if(props.hasOwnProperty(k))
                       {
                           this._checkInputBody(props[k],`${target}.${k}`,objName)
                       }
                   }
               }
           }
