...
class ConfigPeCompiler
{
    ...
    preCompile()
    {
        ...
        this._preCompileChannelConfig();

        //view precompiled app config
        //console.dir(this._zc.getAppConfig(),{depth:null})
        //console.dir(this._zc.getChannelConfig(),{depth:null})
    }
    ...
    _preCompileChannelConfig()
    {
        if(this._zc.isChannel(Const.Channel.KEYS.CUSTOM_CHANNELS))
        {
            this._preCompileChannelDefault(this._zc.getChannel(Const.Channel.KEYS.CUSTOM_CHANNELS));
        }
        else
        {
            this._zc.getChannelConfig()[Const.Channel.KEYS.CUSTOM_CHANNELS] = {};
        }

        if(this._zc.isChannel(Const.Channel.KEYS.CUSTOM_ID_CHANNELS))
        {
            this._preCompileChannelDefault(this._zc.getChannel(Const.Channel.KEYS.CUSTOM_ID_CHANNELS));
        }
        else
        {
            this._zc.getChannelConfig()[Const.Channel.KEYS.CUSTOM_ID_CHANNELS] = {};
        }
    }

    _preCompileChannelDefault(channels)
    {
        if(this._zc.isChannel(Const.Channel.KEYS.DEFAULTS))
        {
            let defaultCh = this._zc.getChannel(Const.Channel.KEYS.DEFAULTS);
