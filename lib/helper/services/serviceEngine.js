...
class ServiceEngine
{
    constructor(zc)
    {
        this._sc = zc.getMain(Const.Main.KEYS.SERVICES);
        this._csc = zc.getMain(Const.Main.KEYS.CUSTOM_SERVICES);
    ...
    async init()
    {
        ...
        this._mySqlServiceBox =
            new ServiceBox(Const.Main.SERVICES.MYSQL,this._sc[Const.Main.SERVICES.MYSQL],async (c) =>
            {
        ...
        this._nodeMailerServiceBox =
            new ServiceBox(Const.Main.SERVICES.NODE_MAILER,this._sc[Const.Main.SERVICES.NODE_MAILER],async (c) =>
            {
        ...
        this._postgresSqlBox =
            new ServiceBox(Const.Main.SERVICES.POSTGRES_SQL,this._sc[Const.Main.SERVICES.POSTGRES_SQL],async (c) =>
            {
        ...
        this._mongoDbBox =
            new ServiceBox(Const.Main.SERVICES.MONGO_DB,this._sc[Const.Main.SERVICES.MONGO_DB],async (c) =>
            {
        ...
        for(let k in this._csc)
        {
            ...
            {
                let howToCreate = this._csc[k][Const.Main.CUSTOM_SERVICES.CREATE];
                let howToGet    = this._csc[k][Const.Main.CUSTOM_SERVICES.GET];
                //remove
                delete this._csc[k][Const.Main.CUSTOM_SERVICES.CREATE];
                delete this._csc[k][Const.Main.CUSTOM_SERVICES.GET];
