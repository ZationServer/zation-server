/*
Author: Luca Scaringella
GitHub: LucaCode
©Copyright by Luca Scaringella
 */

class AppConfig {}

AppConfig.AUTH_CONTROLLER         = 'authController';
AppConfig.CONTROLLER              = 'controller';
AppConfig.GROUPS                  = 'groups';
AppConfig.VERSION_CONTROL         = 'versionControl';
AppConfig.CONTROLLER_DEFAULT      = 'controllerDefault';

AppConfig.SERVER_SOCKET_ACCESS     = 'socketAccess';
AppConfig.SERVER_HTTP_ACCESS       = 'httpAccess';

AppConfig.GROUPS_DEFAULT_GROUP           = 'defaultGroup';
AppConfig.GROUPS_AUTH_GROUPS             = 'authGroups';

AppConfig.CONTROLLER_NAME                = 'name';
AppConfig.CONTROLLER_PATH                = 'path';
AppConfig.CONTROLLER_INPUT               = 'input';
AppConfig.CONTROLLER_BEFORE_HANDLE       = 'beforeHandle';
AppConfig.CONTROLLER_PARAMS_CAN_MISSING  = 'paramsCanMissing';
AppConfig.CONTROLLER_EXTRA_SECURE        = 'extraSecure';
AppConfig.CONTROLLER_SYSTEM_CONTROLLER   = 'systemController';
AppConfig.CONTROLLER_HTTP_ACCESS         = 'httpAccess';
AppConfig.CONTROLLER_SOCKER_ACCESS       = 'socketAccess';
AppConfig.CONTROLLER_INPUT_VALIDATION    = 'inputValidation';

AppConfig.CONTROLLER_NOT_ACCESS          = 'notAccess';
AppConfig.CONTROLLER_ACCESS              = 'access';

AppConfig.ACCESS_ALL_AUTH                = 'allAuth';
AppConfig.ACCESS_ALL_NOT_AUTH            = 'allNotAuth';
AppConfig.ACCESS_ALL                     = 'all';

AppConfig.PARAMS_NAME                    = 'name';
AppConfig.PARAMS_TYPE                    = 'type';
AppConfig.PARAMS_IS_OPTIONAL             = 'isOptional';

module.exports = AppConfig;