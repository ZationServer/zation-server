...
class MasterTempDbEngine
{
    ...
    async init()
    {
        ...
        {
            if(this._zc.getMain(Const.Main.TEMP_DB_ENGINE) === Const.Main.TEMP_DB_ENGINE_MONGO)
            {
                const mongoClient = await MongoClient.connect(this._zc.getMain(Const.Main.TEMP_DB_CONFIG).url);

                this._zc.printStartDebugInfo('Clear mongo temp db');
            ...
            else if(this._zc.getMain(Const.Main.TEMP_DB_ENGINE) === Const.Main.TEMP_DB_ENGINE_MASTER_MEMORY)
            {
                this._zc.printStartDebugInfo('Create master memory temp db');
            ...
            {
                throw new Error(`Invalid tempDb Engine -> ${this._zc.getMain(Const.Main.TEMP_DB_ENGINE)}`);
            }
        }
    }
    ...
    async processMemoryDbReq(data,resp)
    {
        ...
        switch (dbName)
        {
            case Const.Settings.TEMP_DB_TOKEN_INFO_NAME :
                await MasterTempDbEngine._processMemoryDbCommand(this._memoryTokenInfoDb,data,resp);
                break;
            case Const.Settings.TEMP_DB_ERROR_INFO_NAME :
