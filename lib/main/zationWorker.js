...
class Worker extends SCWorker
{
            ...
            if(this._zc.checkMiddlewareEvent(Const.Event.MIDDLEWARE_SUBSCRIBE,req,next))
            {
                ...
                if (authToken !== null) {
                    ...
                    if (channel.indexOf(Const.Settings.CHANNEL_USER_CHANNEL_PREFIX) !== -1) {
                        ...
                        if (ChAccessEngine.hasAccessToSubSpecialChannel(req.socket, chName,this._preapreSmallBag)) {
        ...
        this.scServer.addMiddleware(this.scServer.MIDDLEWARE_PUBLISH_IN, (req, next) => {
            ...
            if(this._zc.checkMiddlewareEvent(Const.Event.MIDDLEWARE_PUBLISH_IN,req,next))
            {
                if (req.channel.indexOf(Const.Settings.CHANNEL_USER_CHANNEL_PREFIX) !== -1) {
                    ...
                    if (ChAccessEngine.hasAccessToPubInSpecialChannel(req.socket, chName,this._preapreSmallBag)) {
                        next();
                    }
                    else {
        ...
        this.scServer.addMiddleware(this.scServer.MIDDLEWARE_AUTHENTICATE, (req,next) =>
        {
            if(this._zc.checkMiddlewareEvent(Const.Event.MIDDLEWARE_AUTHENTICATE,req,next))
            {
                if(this._zc.isExtraSecureAuth())
                {
                    ...
                    this.getTempDbUp().isTokenUnblocked(token[Const.Settings.CLIENT_TOKEN_ID]).then((valid) =>
                    {
                        if(!valid)
                        {
    ...
    _initSocketEvents(socket)
    {
        ...
        {
            if(this._zc.isUseTokenInfoTempDb() && socket.authToken() !== null)
            {
    ...
    async _initTempDb()
    {
        if(this._zc.getMain(Const.Main.TEMP_DB_ENGINE) === Const.Main.TEMP_DB_ENGINE_MASTER_MEMORY)
        {
